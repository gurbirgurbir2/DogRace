...
namespace DogRace
{
    public partial class Form1 : Form
    {
        ...
        public void Punters()
        {
            ...
            myPunters[0].MyLabel = lblJhon;
            myPunters[0].MyRadioButton = rbJhon;
            myPunters[0].MyRadioButton.Text = myPunters[0].Name + " has $" + myPunters[0].Cash;
            myPunters[1].MyLabel = lblPatlu;
            myPunters[1].MyRadioButton = rbPatlu;
        ...
        {
            if (myPunters[0].Cash <= 0)//Jhon
            {
                lblJhon.Text = "Jhon is now broke and cannot continue betting";
                rbJhon.Enabled = false;
            }
            if (myPunters[1].Cash <= 0)//Patlu
            {
                lblPatlu.Text = "Patlu is now broke and cannot continue betting";
                rbPatlu.Enabled = false;
            }
        ...
        private void rbJhon_CheckedChanged(object sender, EventArgs e)
        {
            //Show that Jhon is betting in the bettor label
            lblBettor.Text = myPunters[0].Name;
            // Sets the maximum bet based off cash
            udBoxBet.Maximum = myPunters[0].Cash;
        }

        private void rbPatlu_CheckedChanged(object sender, EventArgs e)
        {
            //Show that Patlu is betting in the bettor label
        ...
        private void btnRace_Click(object sender, EventArgs e)
        {
            ...
            if (myPunters[0].Cash < udBoxBet.Value && rbJhon.Enabled)
            {
                MessageBox.Show("Sorry but Jhon does not have enough cash to proceed.");
                timer1.Enabled = false;
            }
            if (myPunters[1].Cash < udBoxBet.Value && rbPatlu.Enabled)
            {
                MessageBox.Show("Sorry but Patlu does not have enough cash to proceed.");
                timer1.Enabled = false;
            }
        ...
        public void btnLockIn_Click(object sender, EventArgs e) // Unlock the Race Button and check bet amount is not more than cash
        {
            try
            {
                ...
                {
                    MessageBox.Show("Jhon does not have enough to proceed");
                    btnRace.Enabled = false;
                }
                if (myPunters[1].Cash < myPunters[1].myBet.Amount)
                {
                    MessageBox.Show("Patlu does not have enough to proceed");
                    btnRace.Enabled = false;
                }
